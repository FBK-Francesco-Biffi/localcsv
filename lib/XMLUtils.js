...
function writeXML(data, outputDir, keyId = 'name') {
    ...
    const fileMap = createXMLFiles(data[0], keyId)
    data.forEach(row => {
        ...
        Object.keys(row).forEach(key => {
            ...
            if (key !== keyId) {
                const item = fileMap[key].ele('string')
    ...
    Object.keys(fileMap).forEach(key => {
        ...
        const xmlString = file.end({ prettyPrint: true });
...
function createXMLFiles(row, keyId = 'name') {
    const fileMap = {}
    Object.keys(row).forEach(key => {
        if(key !== keyId) {
